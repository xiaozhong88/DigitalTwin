...
import * as ocreanMaterials from "../materials/OceanMaterial";
import { Random } from "../utils/Random";
import { camera } from "../Scene";

const tilesPerAxis = 32; // 网格每个轴上的瓦片数量
const tileSize = 32; // 每个瓦片的大小
const tilesRadius = MathUtils.clamp(8, 1, tilesPerAxis / 2); // 瓦片半径，限制在1到网格每个轴上瓦片数量的一半之间
const tileSize1 = tileSize + 1; // 每个瓦片大小加1
const worldSize = tilesPerAxis * tileSize; // 世界尺寸，即网格的总大小
const worldSize1 = worldSize + 1; // 世界尺寸加1
const scale = 1; // 缩放比例
const halfSize = tilesPerAxis * tileSize * 0.5 * scale; // 世界大小的一半
...
const erosion = new Vector3(0.008, 0.02, 0.1); // 侵蚀参数

const hill = new Vector2(0.03, 100); // 山丘参数
...
const random = new Random(); // 随机数生成器
...
// 三次插值函数，目的生成一个平滑的插值曲线，用于在两个值之间进行平滑的过渡
function CubicInterpolation(a, b, t) {
    t = t * t * t * (t * (t * 6 - 15) + 10); // 插值系数
    return (1 - t) * a + t * b; // 加权插值
}
...
export function Start() {
    ...
    for (let tileZ = 0; tileZ < tilesPerAxis; tileZ++) {
        for (let tileX = 0; tileX < tilesPerAxis; tileX++) {
            ...
            mesh.material = ocreanMaterials.triplanar;
            mesh.visible = false;
